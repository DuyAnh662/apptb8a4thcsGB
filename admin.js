...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.loadData = loadData; // Make it globally accessible

  // Initial Load
  loadData();

  // ------------------ Notifications: send + history + scheduler ------------------
  const notifTitleEl = document.getElementById('notif_title');
  const notifMsgEl = document.getElementById('notif_message');
  const sendNotifBtn = document.getElementById('sendNotifBtn');
  const sendNotifToAllBtn = document.getElementById('sendNotifToAllBtn');
  const createDailyNowBtn = document.getElementById('createDailyNow');
  const notifHistoryEl = document.getElementById('notifHistory');

  async function fetchNotifHistory() {
    if (typeof supabase === 'undefined' || !notifHistoryEl) return;
    try {
      const res = await supabase.from('notification').select('*').order('created_at', { ascending: false }).limit(50);
      const list = (res.data || []);
      notifHistoryEl.innerHTML = list.map(n => {
        const time = n.created_at ? new Date(n.created_at).toLocaleString() : '';
        return `<div style="padding:8px;border-bottom:1px solid rgba(0,0,0,0.05);"><div style="font-weight:700">${escapeHtml(n.title || '—')}</div><div style="font-size:13px;color:var(--text-secondary);">${escapeHtml(n.message || n.content || '')}</div><div style="font-size:11px;color:var(--text-sec);margin-top:6px">${time}</div></div>`;
      }).join('');
    } catch (e) { console.warn('Load notif history failed', e); }
  }
  ...
  async function sendNotificationToDB(title, message, url = '/') {
    try {
      await supabase.from('notification').insert([{ title, message, url }]);
      showToast('Đã lưu thông báo vào DB', 'success');
      await fetchNotifHistory();
    } catch (e) { console.error(e); showToast('Gửi thất bại', 'error'); }
  }

  sendNotifBtn?.addEventListener('click', async () => {
    const t = notifTitleEl.value.trim(); const m = notifMsgEl.value.trim();
    if (!t || !m) return showToast('Vui lòng nhập tiêu đề và nội dung', 'error');

    // Save to DB
    await sendNotificationToDB(t, m, '/');

    // Call push_dispatcher Edge Function to send push notifications
    try {
      const pushResponse = await fetch(SUPABASE_URL + '/functions/v1/push_dispatcher', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          record: { title: t, message: m, url: '/' }
        })
      });
      const pushResult = await pushResponse.json();
      console.log('[Push Result]', pushResult);
      if (pushResult.sent > 0) {
        showToast('Đã gửi đến ' + pushResult.sent + ' thiết bị!', 'success');
      }
    } catch (e) {
      console.warn('[Push Error]', e);
    }

    notifTitleEl.value = ''; notifMsgEl.value = '';
  });

  sendNotifToAllBtn?.addEventListener('click', async () => {
    const t = notifTitleEl.value.trim(); const m = notifMsgEl.value.trim();
    if (!t || !m) return showToast('Vui lòng nhập tiêu đề và nội dung', 'error');
    await sendNotificationToDB(t, m, '/');
    notifTitleEl.value = ''; notifMsgEl.value = '';
  ...
  createDailyNowBtn?.addEventListener('click', async () => {
    await createDailyNotificationNow();
  });

  // Create daily notification based on TKB (tomorrow) and BTVN
  async function createDailyNotificationNow() {
    try {
      ...
      // Calculate tomorrow day index like in script.js
      const now = new Date();
      let day = now.getDay();
      if (now.getHours() >= 16) day++;
      if (day >= 6 || day === 0) day = 1;
      const list = tkb.filter(i => Number(i.day) === day);
      const subjects = [...new Set(list.map(i => (i.subject || '').toLowerCase()))];

      if (!subjects.length) return showToast('Không có môn cho ngày mai', 'info');

      // Collect btvn for those subjects
      const lines = subjects.map(s => {
        const items = btvn.filter(b => (b.subject || '').toLowerCase().includes(s));
        const msgs = items.map(it => it.content || it.note || '').filter(Boolean);
        return `• ${capitalize(s)}: ${msgs.length ? msgs.join(' | ') : 'Không có bài tập'}`;
      });

      const title = 'Bài tập - Ngày mai';
      const message = `Các môn ngày mai:\n${lines.join('\n')}`;
      await sendNotificationToDB(title, message, '/');
    } catch (e) { console.error(e); showToast('Tạo thông báo hằng ngày thất bại', 'error'); }
  }

  // --- Event-based notifications (Tết, Noel, Halloween) ---
  const eventNotifications = [
    {
      name: "Tết Nguyên Đán",
      startDate: { m: 2, d: 16 }, endDate: { m: 2, d: 24 },
      title: "🎊 Chúc mừng Tết Nguyên Đán! 🎊",
      message: "An khang thịnh vượng, vạn sự như ý!\nHappy Lunar New Year! Wishing you success and prosperity! 🧧✨"
    },
    {
      name: "Giáng Sinh",
      startDate: { m: 12, d: 24 }, endDate: { m: 12, d: 26 },
      title: "🎄 Merry Christmas! 🎄",
      message: "Chúc bạn một mùa Giáng sinh an lành, ấm áp và đầy niềm vui!\nWishing you a magical and wonderful Christmas season! ❄️🎁"
    },
    {
      name: "Halloween",
      startDate: { m: 10, d: 30 }, endDate: { m: 11, d: 1 },
      title: "🎃 Happy Halloween! 🎃",
      message: "Trick or Treat! Chúc bạn một Halloween thú vị và đầy màu sắc!\nCẩn thận những con ma quỷ... 👻🦇"
    }
  ];

  async function checkAndSendEventNotification() {
    try {
      if (typeof supabase === 'undefined') return;
      ...
      // Find matching event
      const event = eventNotifications.find(e => {
      ...
      if (!event) return; // No event today

      // Check if already sent today
      const eventKey = `eventNotif_${event.name}`;
      const lastSent = localStorage.getItem(eventKey);
      const todayStr = today.toISOString().slice(0, 10);

      if (lastSent === todayStr) return; // Already sent today

      // Send event notification
      await sendNotificationToDB(event.title, event.message, '/');
      localStorage.setItem(eventKey, todayStr);
  ...
  // --- FREE NOTICE HANDLER ---
  let freeNoticeCounter = 0;
  const freeTitleEl = document.getElementById('free_title');
  const freeMsgEl = document.getElementById('free_message');
  const freeDelayEl = document.getElementById('free_delay_min');
  const sendFreeNoticeBtn = document.getElementById('sendFreeNoticeBtn');

  sendFreeNoticeBtn?.addEventListener('click', async () => {
    const t = freeTitleEl?.value?.trim();
    const m = freeMsgEl?.value?.trim();
    const delay = parseInt(freeDelayEl?.value) || 1;
    if (!t || !m) return showToast('Vui lòng nhập tiêu đề và nội dung', 'error');

    try {
      freeNoticeCounter++;
      const sendAfter = new Date(Date.now() + delay * 60000).toISOString();

      const { error } = await supabase.from('free_notices').insert([{
        seq: freeNoticeCounter,
        title: t,
        message: m,
      ...
      showToast(`✅ Thông báo sẽ gửi trong ${delay} phút`, 'success');
      freeTitleEl.value = '';
      freeMsgEl.value = '';
      freeDelayEl.value = '1';
      await fetchNotifHistory();
  ...
  // Button để test gọi process_free_notice Edge Function
  const testProcessBtn = document.getElementById('testProcessFreeNoticeBtn');
  if (testProcessBtn) {
    testProcessBtn.addEventListener('click', async () => {
      try {
        showToast('Đang gọi process_free_notice...', 'info');
        const { data, error } = await supabase.functions.invoke('process_free_notice', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          }
        ...
        if (error) {
          ...
          showToast('Lỗi: ' + JSON.stringify(error), 'error');
        } else {
          console.log('Function result:', data);
          showToast('Kết quả: ' + JSON.stringify(data), 'success');
        }
      } catch (e) {
...
// NOTE: All automatic notifications (Daily + Event) are now handled by:
// 1. Supabase Edge Function (send-daily-notification) - runs at 18:00 UTC+7 daily via cron-job.org
// 2. Client-side realtime listener (in script.js setupNotifications) - displays notifications as they arrive
// DO NOT add scheduler here - Edge Function handles all automation!
